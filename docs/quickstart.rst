...
This is a quickstart reference to ``pyautogui-rust``. It is a Python 3.9+
cross-platform GUI automation package that preserves the PyAutoGUI API and adds
an optional Rust acceleration layer on Windows.

**All the keyword arguments in the examples on this page are optional.**

    >>> import pyautogui

Install from PyPI with ``python -m pip install pyautogui-rust``. The import
name remains ``pyautogui``.
