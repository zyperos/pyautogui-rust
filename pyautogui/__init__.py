...
try:
    ...
    # OpenCV, NumPy, MSS, and the Rust module are accelerators rather than
    # requirements of PyAutoGUI's established screenshot API. If this backend
    # cannot initialize, retain PyScreeze's original implementation.
    try:
        from . import _pyautogui_pyscreeze_opt
    except Exception:
        _pyautogui_pyscreeze_opt = pyscreeze

    center = pyscreeze.center
    pixel = getattr(_pyautogui_pyscreeze_opt, "pixel", pyscreeze.pixel)
    pixelMatchesColor = getattr(
        _pyautogui_pyscreeze_opt, "pixelMatchesColor", pyscreeze.pixelMatchesColor
    )
    screenshot = getattr(_pyautogui_pyscreeze_opt, "screenshot", pyscreeze.screenshot)
...
@_genericPyAutoGUIChecks
def moveRel3D(xOffset=0, yOffset=0, _pause=True):
    """Sends a raw relative mouse movement packet (without MOUSEEVENTF_ABSOLUTE).
    This is useful for 3D game engines (like DirectX/OpenGL/Vulkan games, e.g. Aim Lab)
    where mouse cursor coordinates are locked to the center and the game engine
    listens to raw delta mouse reports to rotate the camera.

    Args:
      xOffset (int): Relative x movement in pixels/mickeys.
      yOffset (int): Relative y movement in pixels/mickeys.
    """
    ...
    try:
        platformModule._moveRel(xOffset, yOffset)
    except AttributeError:
        raise NotImplementedError("Relative 3D mouse movement is not implemented on this platform.")


moveRelRaw = moveRel3D  # Alias for discoverability
...
def _mouseMoveDrag(moveOrDrag, x, y, xOffset, yOffset, duration, tween=linear, button=None):
    ...
    if duration > MINIMUM_DURATION:
        if moveOrDrag == "move" and tween == linear and hasattr(platformModule, "_moveToSmooth"):
            num_steps = max(width, height)
            sleep_amount = duration / num_steps
            if sleep_amount < MINIMUM_SLEEP:
                num_steps = int(duration / MINIMUM_SLEEP)
            # A native backend can become unavailable after import (for
            # example, a Win32 call can fail at runtime). In that case the
            # wrapper returns False and this function continues through the
            # original PyAutoGUI tween loop below.
            if platformModule._moveToSmooth(x, y, duration, num_steps):
                if (x, y) not in FAILSAFE_POINTS:
                    failSafeCheck()
                return
...
def press(keys, presses=1, interval=0.0, logScreenshot=None, _pause=True):
    ...
    platform_press = getattr(platformModule, "_press", None)
    for i in range(presses):
        for k in keys:
            failSafeCheck()
            if platform_press is not None:
                platform_press(k)
            else:
...
def failSafeCheck():
    ...
    # Preserve the original PyAutoGUI semantics, including user-supplied
    # FAILSAFE_POINTS. The optional Windows hook augments this check by
    # remembering a brief visit to a corner between Python API calls.
    triggered = tuple(position()) in FAILSAFE_POINTS
    if sys.platform == "win32" and hasattr(platformModule, "_consume_failsafe_trigger"):
        triggered = platformModule._consume_failsafe_trigger() or triggered
...
def getBackendInfo():
    """Return a side-effect-free description of optional runtime backends.

    The result is intended for diagnostics, support reports, and deployment
    checks. It never emits input, captures the screen, or initializes a model.
    """

    native_core = sys.modules.get(__name__ + "._rust_core")
    if native_core is None:
        try:
            from . import _rust_core as native_core
        except Exception:
            native_core = None

    native_features = {
        "input": ("send_inputs", "set_cursor_pos"),
        "screenshot": ("capture_screen_gdi",),
        "template_matching": ("locate_on_screen_rust",),
        "visual_location": ("tinylocate_model_info", "tinylocate_infer"),
    }
    available_features = tuple(
        name
        for name, functions in native_features.items()
        if native_core is not None and all(callable(getattr(native_core, function, None)) for function in functions)
    )
    return {
        "package": "pyautogui-rust",
        "version": __version__,
        "platform": sys.platform,
        "native_extension": native_core is not None,
        "windows_acceleration": sys.platform == "win32" and native_core is not None,
        "native_features": available_features,
    }
